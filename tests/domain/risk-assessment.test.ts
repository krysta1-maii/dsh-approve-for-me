...
describe('assessVerifiedActionV1', () => {
  it('records network payload risk without inferring user authorization', () => {
    ...
    expect(assessment).toMatchObject({ risk: 'high', categories: ['data-exfiltration', 'external-side-effect'], authorization: { level: 'unknown', targetCovered: false, sideEffectsCovered: false, sourceRefs: ['event:7'] } })
  ...
  it('marks destructive privilege expansion critical and remains non-authorizing', () => {
    ...
    expect(assessVerifiedActionV1(action, []).categories).toEqual(['destructive-change', 'privilege-expansion'])
