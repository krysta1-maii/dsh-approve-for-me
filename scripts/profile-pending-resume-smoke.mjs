...
const output = safeBuildOutput(root, process.env.PROFILE_PENDING_SMOKE_OUTPUT ?? join(root, '.build/profile-pending-smoke'), 'PROFILE_PENDING_SMOKE_OUTPUT')
const profile = 'approve-for-me-pending-smoke'
const temp = mkdtempSync('/tmp/dsh-approve-pending-')
const dshHome = join(temp, 'home')
const shimDir = join(temp, 'bin')
const cliPrefix = join(temp, 'cli')
const armMarker = join(output, 'probe-arm.json')
const verifyMarker = join(output, 'probe-verify.json')
...
function installTargetCli() {
  ...
  writeFileSync(join(cliPrefix, 'package.json'), `${JSON.stringify({
    name: 'dsh-approve-for-me-profile-pending-smoke-cli',
    private: true,
    version: '0.0.0',
...
try {
  ...
  // Phase A: arm
  rmSync(armMarker, { force: true })
  let phaseAKilled = false
  try {
    run(process.execPath, [cli, '--profile', profile], {
      cwd: temp,
      env: {
        ...env,
        DSH_APPROVE_FOR_ME_PROFILE_PROBE: armMarker,
        DSH_APPROVE_FOR_ME_PROFILE_PROBE_PHASE: 'arm',
        DSH_APPROVE_FOR_ME_PROFILE_PROBE_SESSION: sessionId,
      },
      timeout: 60_000,
    })
  } catch (error) {
  ...
  if (!phaseAKilled) {
    throw new Error('Phase A Profile boot did not terminate with SIGKILL')
  }
  if (!existsSync(armMarker)) {
    throw new Error('Phase A Profile boot exited without probe-arm.json')
  }
  const arm = JSON.parse(readFileSync(armMarker, 'utf8'))
  if (arm.phase !== 'armed' || arm.sessionId !== sessionId || typeof arm.requestId !== 'string' || !arm.requestId || arm.callId === undefined) {
    throw new Error(`invalid Phase A arm probe: ${JSON.stringify(arm)}`)
  }
  ...
  run(process.execPath, [cli, '--profile', profile], {
    ...
    env: {
      ...env,
      DSH_APPROVE_FOR_ME_PROFILE_PROBE: verifyMarker,
      DSH_APPROVE_FOR_ME_PROFILE_PROBE_PHASE: 'verify',
      DSH_APPROVE_FOR_ME_PROFILE_PROBE_SESSION: sessionId,
    },
    timeout: 60_000,
  })
  if (!existsSync(verifyMarker)) {
    throw new Error('Phase B Profile boot exited without probe-verify.json')
  }
  const verify = JSON.parse(readFileSync(verifyMarker, 'utf8'))
  if (
    verify.orphanAsked !== true
    || verify.repaired !== true
    || verify.noLateDecision !== true
    || verify.rearmed !== true
    || verify.sideEffect !== false
    || verify.continued !== true
  ) {
    throw new Error(`invalid Phase B verify probe: ${JSON.stringify(verify)}`)
  }
  if (existsSync(join(output, 'pending-side-effect.txt'))) {
    throw new Error('pending-side-effect.txt was created; guarded side effect was not prevented')
  }
  ...
  console.log('PASS pending approval cross-process cold-resume smoke')
  console.log(`Artifacts: ${output}`)
  completed = true
} finally {
  if (!completed && process.env.PROFILE_PENDING_SMOKE_KEEP_TEMP === '1') {
