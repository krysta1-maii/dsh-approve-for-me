...
> 当前代码状态（2026-08-28）：宿主方案已切换到 **机器决策槽 v2**——本仓库交付“插件本体 + 官方 `dsh-user-approval` 的最小 patch”，`dsh-managed-agent` 保持独立仓库作为依赖插件。patch 已在本仓库成形（overlay + 构建/校验脚本），本体的 `approval-gate` 端口骨架已建立；Guardian 编排、trustEnvelope、breaker、记录留存与真实 0.1.2 Web 验收尚未完成。当前测试仍为 82 项，基于 0.1.1-rc.2 基线。
