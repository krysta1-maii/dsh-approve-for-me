...
# 产物：.build/dsh-user-approval-afm-0.1.2-rc.1.tgz
```
...
当前锁定：`upstreamVersion 0.1.2-rc.1`、`upstreamTag dsh-v0.1.2-rc.1`、`upstreamCommit a66e4702047846cdaa10c66c9d3df3951f5ea70d`、`patchVersion 4`；patch 语义与上一版本一致。rc.1 相对 alpha.5 上游源码零变更（仅版本号翻动），overlay 原样平移；alpha.5 版的上游增量是 Session 日志读取从 `session.events` 迁到 `snapshotEvents()`/`eventAt()`。
