...
import type { ApprovalOutcome, ApprovalRequestEvent, MachineApprovalDecision, MachineApprovalPolicy } from './types.ts'

export { ApprovalRequestId } from './types.ts'
export type { ApprovalOutcome, MachineApprovalDecision, MachineApprovalPolicy } from './types.ts'
...
export class ApprovalService extends Service {
  ...
  /**
   * Resolve the question through the registered machine policies and then the
   * interactive waterfall, contained and raced against the request signal.
   * Machine policies run after the session `never` check and before any
   * answerer, so waterfall listener order cannot preempt a machine decision.
  ...
  private async decide(req: ApprovalRequest, session: Session): Promise<ApprovalOutcome> {
    ...
    // Enter the promise chain BEFORE dispatching: a machine policy or listener
    // that throws SYNCHRONOUSLY (before its first await) must land in the same
    // rejection path as an async one — `Promise.resolve(call())` would let it
    ...
    const answer: Promise<ApprovalOutcome> = Promise.resolve().then(
      ...
      // A throwing machine policy or answerer must fail the QUESTION closed,
      // not the caller's tool call open — the seam contains its callbacks.
      () => 'unavailable',
    )
