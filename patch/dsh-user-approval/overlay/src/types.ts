...
/**
 * One machine-policy answer: a closed outcome claims the request; `'delegate'`
 * declines it so the service continues to the next registered machine policy
 * and then the interactive `approval/request` waterfall.
 */
export type MachineApprovalDecision = ApprovalOutcome | 'delegate'

/**
 * A deterministic, service-level approval policy consulted after the session
 * `never` check and before any interactive answerer. Registration order is
 * dispatch order; the first non-`'delegate'` result claims the request.
 * A throwing policy fails the request closed with `'unavailable'`.
 */
export interface MachineApprovalPolicy {
  /** Stable identity; duplicate ids are rejected at registration. */
  readonly id: string
...
export interface ApprovalRequestEvent {
  ...
  /**
   * Audit id of this ask (pairs `approval/asked` with `approval/decided`).
   * Present on every request dispatched by `ApprovalService.request()`.
   */
