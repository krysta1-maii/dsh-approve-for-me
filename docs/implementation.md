...
当前已有 D1 编译端口、校验、首个确定性编译器、delegation projector 与 in-memory fact repos。插件已从 canonical `tools/pre-execute`/`session/event` 投影 execution facts 与 immutable approval snapshots，并在事实源处以完整 lifecycle identity（含 cwd）筛除不一致 sidecar；机器决策会等待同一 durable `approval/asked` 事件的 snapshot write，消除 fire-and-forget observer 与 resolver 的竞速。已接入 `DossierGateFactProjector`，它仅从 branded dossier 重建 classification、parent lifecycle、turn、direct-user frontier 与 cache keys；缺少 direct-user evidence 时不产生 facts。历史 direct-user evidence 会保留 `visible` 与 `superseded` 状态；已完成 turn 的唯一可见、未中断、纯文本 model assistant message 会连同精确 `turn/end` summary 投影为 principal delivery，任何模糊、非完成或非纯文本证据均失败关闭。完整 instruction/tool 投影、Storage Domain sidecar 与双射验证尚未实现。
