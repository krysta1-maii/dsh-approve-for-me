...
| `src/application/dossier-compiler.ts` | `DefaultDossierCompiler`：校验 principal/authority/execution-fact；严格最小完整前缀可输出 `SourceVerifiedDossierV1` + metrics，任何 assistant/instruction、既有工具、delegation 或未知历史均拒绝而不静默省略 |
