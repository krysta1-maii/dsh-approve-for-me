...
| `src/application/dossier-compiler.ts` | `DefaultDossierCompiler`：校验 principal/authority/execution-fact；严格最小完整前缀可输出 `SourceVerifiedDossierV1` + metrics，并保留最新 canonical `request/header`；`freeze.frozenAt` 严格绑定 canonical `approval/asked` 的事件时间。direct-user 还必须由 canonical surface placement 标记为可见。任何 assistant/instruction、既有工具、delegation、未知或 surface 状态不确定历史均拒绝而不静默省略 |
