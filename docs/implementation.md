...
| 文件 | 职责 |
|---|---|
| `src/domain/records.ts` | `SessionLifecycleIdentityV1`、`GuardianCaseCaptureConfigV1` 校验、`r1_`/`c1_` key 编码、packet/decision/schema/policy 的版本化 hash domain、artifact 计费字节 |

当前仍是纯类型/哈希基础；Storage Domain writer、create-once durable read-back、quota/GC 尚未实现。
...
```bash
npm run check   # 本机 0.1.1-rc.2 安装基线：typecheck + 128 项测试 + build
